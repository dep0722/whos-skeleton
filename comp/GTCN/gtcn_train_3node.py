...
檔名規則: L*.csv 使用左側關節, R*.csv 使用右側關節
...
class GaitDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        X = np.stack(node_list, axis=0).astype(np.float32)  # (3, 2, T) - 只有3個關節

        # 計算膝關節角度 (只有1個角度)
        angle = compute_knee_angle(X[0].T, X[1].T, X[2].T).squeeze()  # (T,)
        edge_attr = np.stack([angle, angle], axis=1).astype(np.float32)  # (T, 2) - 2條邊

        X = torch.tensor(X, dtype=torch.float32).permute(1, 2, 0)  # (C,T,V) -> (2,T,3)
        edge_attr = torch.tensor(edge_attr, dtype=torch.float32)  # (T, 2)
...
if __name__ == "__main__":
    ...
    SAVE_PATH = r"C:\mydata\sf\comp\model\GTCN\gtcn_3node_0213_5min.pth"
    BATCH_SIZE = 4
    EPOCHS = 50
    LEARNING_RATE = 0.001
    DEVICE = 'cuda'
    ...
    # 建立模型
    edge_index = [(0, 1), (1, 2)]  # Hip→Knee→Ankle (單側 2 條邊)
